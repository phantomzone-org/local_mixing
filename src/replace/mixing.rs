...
pub fn butterfly_big(
    ...
    // Pick one random R
    let mut rng = rand::rng();
    let (r, r_inv) = random_id(n as u8, rng.random_range(20..=50)); 
    let mut c = c.clone();
    ...
    let mut acc = merge_combine_blocks(&blocks, n, "./circuits.db", &progress, _total);
...
pub fn abutterfly_big(
    ...
    let mut c = c.clone();
    shoot_random_gate(&mut c, 500_000);
    let (first_r, first_r_inv) = random_id(n as u8, rng.random_range(20..=50));
    ...
    println!("Beginning merge");
    let mut acc =
        merge_combine_blocks(&compressed_blocks, n, "./circuits.db", &progress, _total);
